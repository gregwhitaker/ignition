...
#/ Gets the ARN of the resource created using CloudFormation.
find_arn_created_by_cloudformation() {
    CF_ARN=$(aws --output text cloudformation describe-stack-events --stack-name $1 --region $REGION \
        | grep $2 \
        | grep 'CREATE_COMPLETE' \
        | cut -f 9)
}